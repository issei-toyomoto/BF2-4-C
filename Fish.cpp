...
//コンストラクタ
Fish::Fish() {
	LoadDivGraph("image/Enemy/Enemy_FishAnimation.png",10,6,2,64,64,FishImage);
	WaitTime=0;
	f_PosX=0;
	f_PosY=0;
	Count = 0;
...
void Fish::Update() {
	...
	////出現エリア判定
	//if (playerY > 600 || enemyY > 600) {
	//	while (playerY < 600 && enemyY<600) {
	//		Count++;
	//		Second = Count / 60;
	//		//三秒経過＆魚がいない
...
void Fish::MoveFish() {
	...
	//}
	//攻撃対象が範囲から出たとき
	//else if (playerY < 600 /*&& enemyY < 600*/) {
